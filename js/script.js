$(document).ready(function () {
    ...
    $(".questions-js").click(function () {
        $(this).next().toggleClass('questions__block--active')
    })
    ...
    function statisticsDiagram(arrayCurrency) {
        ...
        arrayCurrency.forEach((currency) => {
            ...
            $(".diagram-labels").append("<div class='diagram-labels__item'><span style='background-color: " + currency.color + "'></span>" + currency.label + " - " + currency.value + "%</div>")
        });
    ...
    $(".statistic").each(function () {
        var amount = 0;
        $(this).find(".statistic__block:not(.inactive)").each(function () {
        ...
        $(this).find(".statistic__block:not(.inactive)").each(function () {
            ...
            console.log(percent)
            $(bar).attr("data-percent", percent).css({
                "height": percent + "%",
    ...
    $(document).on('click', '.autentificathion__block-link--password', function (e) {
        $.fancybox.open({
            touch: false,
            closeExisting: true,
            src: '#password',
